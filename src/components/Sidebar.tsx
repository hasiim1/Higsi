...
export default function Sidebar() {
  ...
  const bottomItems = [{ name: "Settings", href: "/settings", icon: Settings }];
    ...
    <>
      ...
      <header className="md:hidden flex items-center justify-between px-6 py-4 bg-sidebar-dark border-b border-border-dark z-20 sticky top-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-outfit font-bold text-xl tracking-tight text-text-primary">
            Higsi
          </span>
        </div>
        <button
                ...
                >
                  <Icon
                    ...
                    className={
                      isActive
                        ? "text-white"
                        : "text-text-secondary group-hover:text-text-primary transition"
                    }
                  />
                  <span>{item.name}</span>
                </Link>
        ...
        <div className="flex flex-col gap-6">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-outfit font-semibold text-sm text-text-primary truncate group-hover:text-primary transition">
                {profile?.name || "Loading..."}
              </h4>
              <p className="font-outfit font-bold text-[10px] text-primary tracking-wider uppercase mt-0.5">
